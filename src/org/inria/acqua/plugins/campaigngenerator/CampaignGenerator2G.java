...
public class CampaignGenerator2G implements Pipelineable, MeasurementReceiver{
	private static Logger logger = Logger.getLogger(CampaignGenerator2G.class.getName()); 
    ...
    public void insertFlowElement(FlowElement fe, String signature) throws Exception{
        ...
        collected = 0;
        logger.info("Pinging started (id "+campaignID+")\n");
        ...
        for (int i=0; i<landmarks_list.size(); i++){
            ...
            logger.info("Created ParallelPinger id " + i + " landmark " + landmark.getDescriptiveName());
    ...
    public synchronized void insertMeasurement(int campaignID, int id, Timestamp[][] pings) throws Exception{
        if (campaignID == this.campaignID){
            ...
            logger.info("Collected ID " + campaignID + ":" + id + " so we have " + collected + " of " + landmarks_list.size() + "\n");
        }else{
            logger.info("Error... Discarding old ParallelPinger input " + id + "\n");
        }
    }
