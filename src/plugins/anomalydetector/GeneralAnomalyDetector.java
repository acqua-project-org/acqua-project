...
//                }
        ...
        /****************************************************/
        }else if(signature.startsWith(PipDefs.SIGN_ANDETLAN)){ 
            {
                ...
                if (currentLandmarkFlowElements.get(landmark)==null){ /* We didn't collect it yet. */
                    ...
                    MyLog.logP(this, "*** Inserted landmark %s FE (%d/%d elements).\n", landmark.getDescriptiveName(), currentLandmarkFlowElements.size(), this.currentLandmarksList.size());
                }else{
                    MyLog.logP(this, "*** ERROR: FE landmark already received ('%s') .\n", landmark.getDescriptiveName());
                    throw new Exception("Landmark already received '" + landmark + "'.");
                }
            }
            if (allLandmarkOutputsCollected()==true){
                double signifLevelSummatory = 0;
                MyLog.logP(this, "+++ All landmarks collected!!!\n\n");
    ...
    private boolean allLandmarkOutputsCollected(){
        for (Landmark l:currentLandmarksList){
            ...
            if (f==null){
                MyLog.logP(this, "+++ Remaining FlowElement of '%s'\n\n", l);
                return false;
            }
        }
        return true;
