...
//        upperIndex = 0.9;
    ...
    private float obtainParticularCurrentRTT(Timestamp[][] timestamps, Integer count) throws NotEnoughDataException{
        ...
        MyLog.appendPToLogFile(this, "Valid RTTs:%d (Avg:%f)\n", valid_ones, rtt_ms_acum);
        if (valid_ones==0){
    ...
    private float obtainGeneralCurrentRTT(FlowElement fe) throws Exception{
        ...
        if (pings_count!=ts_landmark.length){
            MyLog.log(this, "ERROR: The amount of pings measured is not equal to count (landmark " + this.landmark + " index " + landmarkIndex + ").");
        ...
        MyLog.logP(this, "Getting current RTT (%s)...\n", this.landmark.getDescriptiveName());
        try{
            currRTT = obtainParticularCurrentRTT(ts_landmark, pings_count); /* Current RTT. */
        }catch(NotEnoughDataException e){
            MyLog.logP(this, " All failed...TT \n");
    ...
    private synchronized void takeSnapshotAsNormalNow() throws Exception{
        ...
        if (validHistoryRTT.size()>0){
            ...
            System.err.println("Not enough valid samples to make a profile [AnomDB for '" + landmark.getDescriptiveName() + "'].");
            MyLog.log(this, "Not enough valid samples to make a profile [AnomDB for '" + landmark.getDescriptiveName() + "'].");
        }
    }
